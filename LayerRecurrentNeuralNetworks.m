% Layer-Recurrent Neural Networks (layrecnet)
...
minHN = 11; % Минимальное количесвто скрытых нейронов
maxHN = 12; % Максимальное количесвто скрытых нейронов
trainDelayArr = [1]; % Массив возможных задержек
maxEpochs = 1000; 
maxLoop = 600; % Максимальное количество циклов поиска лучшей нейросети
...
if size(OUT, 1) == 1 % Если выход 1, то выводим график, иначе будет белеберда
    ...
    if iscell(OUT) % Выход
        plot(cell2mat(OUT), 'LineWidth',2, 'DisplayName','Target'); hold on; 
        plot(cell2mat(y), 'DisplayName', sprintf('HN=%i R=%0.3f', Result(bestIdx, 1), bestVal))
    else
        plot(OUT, 'LineWidth',2); hold on; 
        plot(y)
    end
    %legend('Target', sprintf('HN=%i R=%0.3f', Result(bestIdx, 1), bestVal))
    legend('show')
    title(ResultStr)
end
