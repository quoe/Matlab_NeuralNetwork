% Pattern Recognition Neural Network (patternnet)
...
minHN = 5; % Минимальное количесвто скрытых нейронов
maxHN = 12; % Максимальное количесвто скрытых нейронов
trainDelayArr = [0]; % Массив возможных задержек
maxEpochs = 1000; 
...
for i = 1:maxLoop
    ...
    nnet = patternnet(HN); % Создание нейросети patternnet
    ...
    nnet = train(nnet, IN, OUT); %  Тренировка нейросети
    y = nnet(IN); % Выходной ответ нейросети
    %plot(cell2mat(y))
    perf = perform(nnet,OUT, y); % Ошибка на выходе нейросети и данных
    r = mean(regression(OUT, y)); % Значение регрессии нейросети и данных
...
if size(OUT, 1) == 1 % Если выход 1, то выводим график, иначе будет белеберда
    figOutput = figure('Name','Target and RNN output');
    figure(figOutput); % figOutput figRegress
    if iscell(OUT) % Выход
        plot(cell2mat(OUT), 'LineWidth',2); hold on; 
        plot(cell2mat(y))
    else
        plot(OUT, 'LineWidth',2); hold on; 
        plot(y)
    end
    legend('Terget', sprintf('HN=%i R=%0.3f', Result(bestIdx, 1), bestVal))
    title(ResultStr)
end
figRegress = figure('Name','Target and RNN regression');
figure(figRegress); % figOutput figRegress
plotregression(OUT, y, ResultStr)
figure, plotconfusion(t,y, ResultStr) % Plot classification confusion matrix
figure, plotroc(t,y) % Plot receiver operating characteristic
view(bestNet)
%vec2ind и ind2vec (цифры разбить на классы (из 0 и 1, т.е. 1 из N) и обратно)
