...
trainInputDelayArr = [2]; % Массив возможных задержек
trainFeedbackDelayArr = [2]; % Массив возможных задержек
maxEpochs = 1000; 
maxLoop = 30; % Максимальное количество циклов поиска лучшей нейросети
...
for i = 1:maxLoop
    ...
    trainInputDelayRand = round((size(trainInputDelayArr,2)-1).*rand(1) + 1); % Случайная задержка на входе
    trainFeedbackDelayRand = round((size(trainFeedbackDelayArr,2)-1).*rand(1) + 1); % Случайная задержка обратной связи
    nnet = narxnet(1:trainInputDelayArr(trainInputDelayRand),1:trainFeedbackDelayArr(trainFeedbackDelayRand), HN); % Создание нейросети narxnet
    ...
    Result = [Result; HN perf r trainRand trainInputDelayArr(trainInputDelayRand) trainFeedbackDelayArr(trainFeedbackDelayRand) toc];
    ...
    if mod(i, autoSaveEvery) == 0
        ...
        fprintf('iter = %i; ', i);
        ResultStr = sprintf('№ %i; HN = %i; DelayI = %i; DelayF = %i; trainParam = %s; R = %0.3f; Perf = %0.3f; Time = %0.3f;', bestIdx, Result(bestIdx, 1), Result(bestIdx, 5), Result(bestIdx, 6), trainParamStr(Result(bestIdx, 4)), bestVal, Result(bestIdx, 2), Result(bestIdx, 7));
...
ResultStr = sprintf('№ %i; HN = %i; DelayI = %i; DelayF = %i; trainParam = %s; \nR = %0.3f; Perf = %0.3f; Time = %0.3f;', bestIdx, Result(bestIdx, 1), Result(bestIdx, 5), Result(bestIdx, 6), trainParamStr(Result(bestIdx, 4)), bestVal, Result(bestIdx, 2), Result(bestIdx, 7));
if size(Ts, 1) == 1 % Если выход 1, то выводим график, иначе будет белеберда
    figOutput = figure('Name','Target and NARX output');
...
figRegress = figure('Name','Target and NARX regression');
